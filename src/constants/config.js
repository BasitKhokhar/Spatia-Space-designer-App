...
// Offline fallback only. When a backend is configured these are overwritten by
// GET /credits (driven by the server's CREDITS_* env vars), so changing them
// here alone does nothing for remote users — the backend must match.
export const CREDITS = {
  starting: 12, // credits a new account begins with
  perAd: 1, // credits earned per rewarded ad
  dailyAdCap: 15, // max rewarded ads per day
  costs: {
    png: 2,
    pdf: 3,
    obj: 5,
